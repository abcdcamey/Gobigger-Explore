...
#                 dis = (v[0]["position"]-my_clone_balls[0]["position"]).length()
...
#                     target_radius>my_clone_balls[0]["radius"] and target_radius*3 > dis:
#                     dangerous_status = 2
#                     safe_direction = (my_clone_balls[0]['position'] - v[0]['position']).normalize()
#                     safe_direction_list.append(safe_direction)
...
class MyBotAgent(BaseAgent):
    ...
    def step_level_3(self, global_state, obs):
        ...
        if len(my_clone_balls) >= 9 and my_clone_balls[4]['radius'] >= 13:
