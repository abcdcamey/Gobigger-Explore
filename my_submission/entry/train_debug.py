...
def main(cfg,ckpt_path=None, seed=0, max_iterations=int(1e10)):
    ...
    collector_env = SyncSubprocessEnvManager(
        env_fn=[lambda: MyGoBiggerEnvV1(collector_env_cfg) for _ in range(collector_env_num)], cfg=cfg.env.manager
    )
    rule_evaluator_env = SyncSubprocessEnvManager(
        env_fn=[lambda: MyGoBiggerEnvV1(evaluator_env_cfg) for _ in range(evaluator_env_num)], cfg=cfg.env.manager
    )
